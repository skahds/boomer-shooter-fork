...
local function GMakeCollectLibs(target)
  local libs = ""
...
local function GMakeCollectDep(target)
  local dep = ""
...
local function GMakeCollectDepFiles(target)
  local dep = ""
  for _, v in ipairs(target.opt.src or {}) do
    dep = dep .. v .. " "
  end
  return dep
end

local function GMakeCollectDepTargets(target)
  local dep = ""
  for _, v in ipairs(target.target_deps) do
    dep = dep .. v .. " "
  end
  return dep
end
...
local function GMakeCompile(target)
  local src = target.output .. ": $(DEP) $(LIBS)\n"
  src = src .. "\t@$(MKDIR) $(@D)\n"
...
local function GMakeClean(target)
  local clean = "clean:\n"
  clean = clean .. "\t$(RM) " .. target.output .. " $(DEP)\n"
...
local function GenerateTarget(target)
  ...
  src = src .. GMakeVar("LIBS", GMakeCollectLibs(target))
  src = src .. GMakeVar("DEP", GMakeCollectDep(target))
...
local function GenerateGMake()
  ...
  for _, target in pairs(ctx.targets) do
    local deps = GMakeCollectDepTargets(target)
    local files = GMakeCollectDepFiles(target)
